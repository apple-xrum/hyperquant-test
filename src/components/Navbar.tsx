import { useLocation, useNavigate } from "react-router-dom";
import { menuItems } from "@constants/menu.ts";

const Navbar = () => {
  const location = useLocation();
  const navigate = useNavigate();

  return (
    <nav className="flex w-full items-stretch justify-between bg-[#232C3E] px-2">
      {menuItems.map((item) => (
        <button key={item.path} className="p-2" onClick={() => navigate(item.path)}>
          <div className="bg-primary-light relative mx-auto aspect-square w-7">
            {item.path === "/profile" && (
              <div className="absolute -top-3 -right-3 flex aspect-square w-6 items-center justify-center rounded-full bg-amber-400 font-bold text-white">
                3
              </div>
            )}
          </div>
          <p
            className={`${location.pathname === item.path ? "text-white" : "text-primary-light"} text-sm`}
          >
            {item.name}
          </p>
        </button>
      ))}
    </nav>
  );
};

export default Navbar;
